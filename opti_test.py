"""
OptiKey keyboard click sequencer (verification tool).

Purpose:
- Move the mouse to key positions over the OptiKey on-screen keyboard and click
    each in sequence. Useful for checking calibration & mapping accuracy.

Two modes:
1) Box grid (uniform rows/cols): simple equal grid inside a captured/specified box.
2) Row-spec (per-row counts): interactively capture the leftmost/rightmost key centers
     per row (e.g., 10,9,7) and we interpolate the centers across each row.

Usage (PowerShell):
    python opti_test.py                             # interactive box capture, default 4x10 grid
    python opti_test.py --rows 10 --cols 4 --delay 0.5 --click-delay 0.2
    python opti_test.py --bbox 200,700,1720,1030    # left,top,right,bottom
    python opti_test.py --rowspec 10,9,7            # interactive per-row capture for 3 rows
    python opti_test.py --rowspec 10,9,7 --preview  # move only, no click

Instructions:
- For box mode, you'll hover the cursor over TOP-LEFT then BOTTOM-RIGHT of OptiKey.
- For rowspec mode, for each row you hover LEFTMOST key center then RIGHTMOST key center.
"""
...
def grid_points(bbox: Tuple[int, int, int, int], rows: int, cols: int, margin: float = 0.08) -> List[Tuple[int, int]]:
    ...
    w = r - l
    h = b - t
    # Apply a small inward margin so we target within key faces
    mx = int(w * margin)
    my = int(h * margin)
...
def rowspec_points(rowspec: Sequence[int]) -> List[Tuple[int, int]]:
    """Interactively capture per-row endpoints and interpolate key centers.

    rowspec: e.g., [10, 9, 7] for three rows.
    """
